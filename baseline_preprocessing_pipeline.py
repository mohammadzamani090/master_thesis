...
class PreprocessingConfig:
    ...
    minimum_retained_epochs_warning: int = 10
    minimum_retained_fraction_warning: float = 0.67
    zero_retained_epochs_is_failure: bool = True

    raw_noise_welch_window_seconds: float = 4.0
    raw_noise_welch_overlap_fraction: float = 0.5
    raw_line_frequency_hz: float = 50.0
    raw_line_band_half_width_hz: float = 0.5
    raw_neighbor_low_band_hz: tuple[float, float] = (45.0, 48.0)
    raw_neighbor_high_band_hz: tuple[float, float] = (52.0, 55.0)
    raw_line_noise_high_ratio_db: float = 10.0
...
class ProcessingSummary:
    ...
    retained_epochs: int
    quality_status: str
    effective_fs: float = float("nan")
    retention_fraction: float = 0.0
    manual_review_required: bool = False
    quality_status_reasons: list[str] = field(default_factory=list)
    raw_noise_metrics: dict[str, Any] = field(default_factory=dict, repr=False)
    rejection_reasons: dict[int, list[str]] = field(default_factory=dict, repr=False)
...
def validate_config(config: PreprocessingConfig) -> None:
    ...
    if config.minimum_retained_epochs_warning < 0:
        raise ValueError("minimum_retained_epochs_warning cannot be negative.")
    if not 0 <= config.minimum_retained_fraction_warning <= 1:
        raise ValueError("minimum_retained_fraction_warning must be between 0 and 1.")
    if config.raw_noise_welch_window_seconds <= 0:
        raise ValueError("raw_noise_welch_window_seconds must be positive.")
    if not 0 <= config.raw_noise_welch_overlap_fraction < 1:
        raise ValueError("raw_noise_welch_overlap_fraction must be in [0, 1).")
    if config.raw_line_frequency_hz <= 0:
        raise ValueError("raw_line_frequency_hz must be positive.")
    if config.raw_line_band_half_width_hz <= 0:
        raise ValueError("raw_line_band_half_width_hz must be positive.")
    for name, band in (
        ("raw_neighbor_low_band_hz", config.raw_neighbor_low_band_hz),
        ("raw_neighbor_high_band_hz", config.raw_neighbor_high_band_hz),
    ):
        if len(band) != 2 or band[0] < 0 or band[1] <= band[0]:
            raise ValueError(f"{name} must be an increasing pair of non-negative frequencies.")
...
# raw_noise_metrics: objective pre-filtering 50 Hz and amplitude diagnostics.
def _prepare_metric_data(eeg_data: np.ndarray) -> np.ndarray:
    """Copy, repair non-finite samples, and constant-demean EEG for QC metrics."""
    metric_data = np.asarray(eeg_data, dtype=float).copy()
    if metric_data.ndim != 2 or metric_data.shape[0] == 0 or metric_data.shape[1] == 0:
        raise ValueError(
            "Raw noise metrics require a non-empty channels-by-samples EEG array."
        )

    for channel_idx, channel in enumerate(metric_data):
        finite = channel[np.isfinite(channel)]
        fill_value = float(np.median(finite)) if finite.size else 0.0
        metric_data[channel_idx] = np.nan_to_num(
            channel,
            nan=fill_value,
            posinf=fill_value,
            neginf=fill_value,
        )
    metric_data -= np.mean(metric_data, axis=-1, keepdims=True)
    return metric_data
...
def _mean_psd_in_band(
    ...
    """Return mean PSD in an inclusive frequency band for each channel."""
    mask = (frequencies >= band[0]) & (frequencies <= band[1])
    if not np.any(mask):
        raise PreprocessingError(
            f"Welch PSD has no bins in requested band {band[0]:.3f}-{band[1]:.3f} Hz."
        )
    return np.mean(psd[..., mask], axis=-1)


def _calculate_channel_noise_metrics(
    eeg_data: np.ndarray,
    ch_names: list[str],
    processing_fs: float,
    config: PreprocessingConfig,
) -> tuple[dict[str, dict[str, Any]], dict[str, Any]]:
    """Calculate channel metrics and a compact aggregate for one EEG interval."""
    if eeg_data.shape[0] != len(ch_names):
        raise ValueError(
            f"EEG channel count {eeg_data.shape[0]} does not match {len(ch_names)} names."
        )
    if config.raw_neighbor_high_band_hz[1] >= processing_fs / 2:
        raise PreprocessingError(
            "Raw noise metric bands must remain below the processing Nyquist frequency."
        )

    scipy_signal = require_module("scipy.signal")
    metric_data = _prepare_metric_data(eeg_data)
    requested_nperseg = int(round(config.raw_noise_welch_window_seconds * processing_fs))
    nperseg = min(requested_nperseg, metric_data.shape[-1])
    if nperseg < 2:
        raise PreprocessingError("Raw noise metric interval is too short for Welch PSD.")
    noverlap = min(
        nperseg - 1,
        int(round(nperseg * config.raw_noise_welch_overlap_fraction)),
    )
    ...
    line_band = (
        config.raw_line_frequency_hz - config.raw_line_band_half_width_hz,
        config.raw_line_frequency_hz + config.raw_line_band_half_width_hz,
    )
    line_power = _mean_psd_in_band(frequencies, psd, line_band)
    low_power = _mean_psd_in_band(
        frequencies,
        psd,
        config.raw_neighbor_low_band_hz,
    )
    high_power = _mean_psd_in_band(
        frequencies,
        psd,
        config.raw_neighbor_high_band_hz,
    )
    neighboring_power = (low_power + high_power) / 2.0
    epsilon = np.finfo(float).tiny
    line_ratio = np.maximum(line_power, epsilon) / np.maximum(
        neighboring_power,
        epsilon,
    )
    line_ratio_db = 10.0 * np.log10(line_ratio)
    peak_to_peak = np.ptp(metric_data, axis=-1)
    standard_deviation = np.std(metric_data, axis=-1)
    high_line_noise = line_ratio_db > config.raw_line_noise_high_ratio_db

    channel_metrics: dict[str, dict[str, Any]] = {}
    for channel_idx, ch_name in enumerate(ch_names):
        channel_metrics[ch_name] = {
            "line_power_psd_v2_per_hz": float(line_power[channel_idx]),
            "neighbor_low_power_psd_v2_per_hz": float(low_power[channel_idx]),
            "neighbor_high_power_psd_v2_per_hz": float(high_power[channel_idx]),
            "neighboring_power_psd_v2_per_hz": float(neighboring_power[channel_idx]),
            "line_noise_power_ratio": float(line_ratio[channel_idx]),
            "line_noise_ratio_db": float(line_ratio_db[channel_idx]),
            "high_line_noise": bool(high_line_noise[channel_idx]),
            "raw_peak_to_peak_volts": float(peak_to_peak[channel_idx]),
            "raw_peak_to_peak_microvolts": float(peak_to_peak[channel_idx] * 1e6),
            "raw_standard_deviation_volts": float(standard_deviation[channel_idx]),
            "raw_standard_deviation_microvolts": float(
                standard_deviation[channel_idx] * 1e6
            ),
        }

    high_count = int(np.sum(high_line_noise))
    summary = {
        "median_line_noise_ratio_db": float(np.median(line_ratio_db)),
        "maximum_line_noise_ratio_db": float(np.max(line_ratio_db)),
        "high_line_noise_channel_count": high_count,
        "high_line_noise_channel_fraction": high_count / len(ch_names),
        "median_peak_to_peak_volts": float(np.median(peak_to_peak)),
        "maximum_peak_to_peak_volts": float(np.max(peak_to_peak)),
        "median_peak_to_peak_microvolts": float(np.median(peak_to_peak) * 1e6),
        "maximum_peak_to_peak_microvolts": float(np.max(peak_to_peak) * 1e6),
    }
    return channel_metrics, summary


def calculate_raw_noise_metrics(
    eeg_data: np.ndarray,
    ch_names: list[str],
    processing_fs: float,
    config: PreprocessingConfig,
) -> dict[str, Any]:
    """Calculate raw recording and aligned epoch QC metrics without modifying EEG."""
    input_shape = eeg_data.shape
    channel_metrics, recording_summary = _calculate_channel_noise_metrics(
        eeg_data,
        ch_names,
        processing_fs,
        config,
    ...
    overlap_samples = int(round(config.epoch_overlap_seconds * processing_fs))
    step = samples_per_epoch - overlap_samples
    if samples_per_epoch <= 0 or step <= 0:
        raise ValueError("Epoch duration and overlap do not produce a positive step.")
    starts = range(0, eeg_data.shape[-1] - samples_per_epoch + 1, step)

    epoch_metrics: list[dict[str, Any]] = []
    for epoch_idx, start in enumerate(starts):
        epoch_data = eeg_data[:, start : start + samples_per_epoch]
        _, epoch_summary = _calculate_channel_noise_metrics(
            epoch_data,
            ch_names,
            processing_fs,
            config,
        ...
        epoch_metrics.append(
            {
                ...
                "median_line_noise_ratio_db": epoch_summary[
                    "median_line_noise_ratio_db"
                ],
                "maximum_line_noise_ratio_db": epoch_summary[
                    "maximum_line_noise_ratio_db"
                ],
                "median_peak_to_peak_volts": epoch_summary[
                    "median_peak_to_peak_volts"
                ],
                "maximum_peak_to_peak_volts": epoch_summary[
                    "maximum_peak_to_peak_volts"
                ],
                "median_peak_to_peak_microvolts": epoch_summary[
                    "median_peak_to_peak_microvolts"
                ],
                "maximum_peak_to_peak_microvolts": epoch_summary[
                    "maximum_peak_to_peak_microvolts"
                ],
                "high_line_noise_channel_count": epoch_summary[
                    "high_line_noise_channel_count"
                ],
            }
        )

    assert eeg_data.shape == input_shape
    return {
        "recording_summary": recording_summary,
        "channel_metrics": channel_metrics,
        "epoch_metrics": epoch_metrics,
        "parameters": {
            "sampling_frequency_hz": processing_fs,
            "welch_window_seconds": config.raw_noise_welch_window_seconds,
            "welch_overlap_fraction": config.raw_noise_welch_overlap_fraction,
            "line_frequency_hz": config.raw_line_frequency_hz,
            "line_band_half_width_hz": config.raw_line_band_half_width_hz,
            "neighbor_low_band_hz": config.raw_neighbor_low_band_hz,
            "neighbor_high_band_hz": config.raw_neighbor_high_band_hz,
            "high_ratio_threshold_db": config.raw_line_noise_high_ratio_db,
        },
    }
...
def determine_recording_quality_status(
    total_generated_epochs: int,
    retained_epoch_count: int,
    warning_messages: list[str],
    config: PreprocessingConfig,
) -> tuple[str, float, bool, list[str]]:
    """Combine processing warnings with configurable epoch-retention quality rules."""
    if total_generated_epochs < 0 or retained_epoch_count < 0:
        raise ValueError("Epoch counts cannot be negative.")
    if retained_epoch_count > total_generated_epochs:
        raise ValueError("Retained epoch count cannot exceed total generated epochs.")

    retention_fraction = (
        retained_epoch_count / total_generated_epochs
        if total_generated_epochs
        else 0.0
    )
    reasons: list[str] = []
    if warning_messages:
        reasons.append("processing_warnings_present")

    if retained_epoch_count == 0 and config.zero_retained_epochs_is_failure:
        reasons.append("zero_retained_epochs")
        return (
            "FAILED_OR_REQUIRES_MANUAL_REVIEW",
            retention_fraction,
            True,
            reasons,
        )
    ...
    if retained_epoch_count > 0:
        if retained_epoch_count < config.minimum_retained_epochs_warning:
            reasons.append(
                "retained_epochs_below_minimum: "
                f"{retained_epoch_count} < {config.minimum_retained_epochs_warning}"
            )
            manual_review_required = True
        if retention_fraction < config.minimum_retained_fraction_warning:
            reasons.append(
                "retention_fraction_below_minimum: "
                f"{retention_fraction:.6g} < "
                f"{config.minimum_retained_fraction_warning:.6g}"
            )
            manual_review_required = True

    quality_status = (
        "OK_WITH_WARNINGS"
        if warning_messages or manual_review_required
        else "OK"
    )
    return quality_status, retention_fraction, manual_review_required, reasons
...
def generate_qc_report(
    ...
    manual_review_required: bool = False,
    quality_status_reasons: list[str] | None = None,
    raw_noise_metrics: dict[str, Any] | None = None,
) -> Path:
    ...
    if retention_fraction is None:
        retention_fraction = retained_epoch_count / total_epochs if total_epochs else 0.0
    report = {
        ...
        "quality_status_reasons": quality_status_reasons or [],
        ...
        "raw_noise_metrics": raw_noise_metrics or {
            "recording_summary": {},
            "channel_metrics": {},
            "epoch_metrics": [],
        },
...
def dry_run_recording(
    ...
    if config.zapline_enabled:
        LOGGER.info("Would run EEG ZapLine at harmonics <= %.1f Hz.", config.eeg_high_cut_hz)
    elif config.fallback_notch_enabled:
        LOGGER.info(
            "Would skip EEG ZapLine and apply the configured notch fallback at %.1f Hz.",
            config.line_noise_hz,
        )
    else:
        LOGGER.info("Would skip EEG ZapLine and notch filtering by configuration.")
    LOGGER.info(
        "Would filter EEG %.1f-%.1f Hz and EOG low-pass %.1f Hz.",
    ...
    return ProcessingSummary(
        ...
        effective_fs=recording.effective_fs,
        retention_fraction=1.0 if proposed_epochs else 0.0,
    )
...
def process_recording(
    ...
    # raw_noise_metrics: analyze a copied centered interval before any channel repair
    # or line-noise/band-pass filtering. These diagnostics never alter raw._data.
    raw_metric_crop = crop_recording(
        raw.get_data(),
        recording.timestamps,
        config,
        processing_fs,
    )
    raw_noise_metrics = calculate_raw_noise_metrics(
        raw_metric_crop.data[groups.eeg_indices],
        groups.eeg_ch_names,
        processing_fs,
        config,
    )
    ...
    (
        ...
        quality_status_reasons,
    ) = determine_recording_quality_status(
        total_generated_epochs=int(epochs.eeg_epochs.shape[0]),
        retained_epoch_count=len(rejection.retained_indices),
    ...
    qc_report_path = generate_qc_report(
        ...
        retention_fraction=retention_fraction,
        manual_review_required=manual_review_required,
        quality_status_reasons=quality_status_reasons,
        raw_noise_metrics=raw_noise_metrics,
        extra={
...
def write_failure_report(
    ...
    report = {
        ...
        "manual_review_required": True,
        "quality_status_reasons": ["processing_failure"],
        "error": str(error),
        "config": config,
    }
    with report_path.open("w", encoding="utf-8") as handle:
...
BATCH_QC_COLUMNS = [
    ...
    "interpolated_channel_names",
    "median_raw_50hz_ratio_db",
    "maximum_raw_50hz_ratio_db",
    "high_50hz_channel_fraction",
    "median_raw_peak_to_peak_microvolts",
    "maximum_raw_peak_to_peak_microvolts",
]
...
def parse_baseline_filename(input_path: Path | str) -> dict[str, str]:
    """Parse subject/session/run/baseline identifiers without failing processing."""
    filename = Path(input_path).name
    match = re.match(
        r"^sub-(?P<subject>[^_]+)_ses-(?P<session>[^_]+)_run-(?P<run>[^_]+)_bl-(?P<baseline>[^_.]+)",
        filename,
        flags=re.IGNORECASE,
    )
    if not match:
        LOGGER.warning("Could not parse baseline identifiers from filename: %s", filename)
        return {"subject": "", "session": "", "run": "", "baseline_number": ""}
    parsed = match.groupdict()
    return {
        "subject": parsed["subject"],
        "session": parsed["session"],
        "run": parsed["run"],
        "baseline_number": parsed["baseline"],
    }


def build_batch_qc_rows(
    summary: ProcessingSummary,
) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    """Build one recording row and all original-epoch rows from compact metadata."""
    identifiers = parse_baseline_filename(summary.input_path)
    raw_summary = summary.raw_noise_metrics.get("recording_summary", {})
    recording_row = {
        "filename": summary.input_path.name,
        **identifiers,
        "original_duration": summary.original_duration,
        "effective_sampling_frequency": summary.effective_fs,
        "total_epochs": summary.total_epochs,
        "retained_epochs": summary.retained_epochs,
        "rejected_epochs": summary.rejected_epochs,
        "retention_fraction": summary.retention_fraction,
        "quality_status": summary.quality_status,
        "manual_review_required": summary.manual_review_required,
        "interpolated_channel_count": len(summary.interpolated_channels),
        "interpolated_channel_names": ";".join(summary.interpolated_channels),
        "median_raw_50hz_ratio_db": raw_summary.get("median_line_noise_ratio_db"),
        "maximum_raw_50hz_ratio_db": raw_summary.get("maximum_line_noise_ratio_db"),
        "high_50hz_channel_fraction": raw_summary.get(
            "high_line_noise_channel_fraction"
        ),
        "median_raw_peak_to_peak_microvolts": raw_summary.get(
            "median_peak_to_peak_microvolts"
        ),
        "maximum_raw_peak_to_peak_microvolts": raw_summary.get(
            "maximum_peak_to_peak_microvolts"
        ),
    }

    epoch_rows: list[dict[str, Any]] = []
    for epoch_metric in summary.raw_noise_metrics.get("epoch_metrics", []):
        epoch_idx = int(epoch_metric["original_epoch_index"])
        reasons = summary.rejection_reasons.get(epoch_idx, [])
        epoch_rows.append(
            {
                "filename": summary.input_path.name,
                **identifiers,
                "epoch_index": epoch_idx,
                "epoch_status": "rejected" if reasons else "retained",
                "median_raw_50hz_ratio_db": epoch_metric.get(
                    "median_line_noise_ratio_db"
                ),
                "maximum_raw_50hz_ratio_db": epoch_metric.get(
                    "maximum_line_noise_ratio_db"
                ),
                "median_raw_peak_to_peak_microvolts": epoch_metric.get(
                    "median_peak_to_peak_microvolts"
                ),
                "maximum_raw_peak_to_peak_microvolts": epoch_metric.get(
                    "maximum_peak_to_peak_microvolts"
                ),
                "high_line_noise_channel_count": epoch_metric.get(
                    "high_line_noise_channel_count"
                ),
                "rejection_reasons": " | ".join(reasons),
            }
        )
    return recording_row, epoch_rows
...
def _write_csv_atomic(
    ...
    """Atomically replace a CSV so the previous completed rows remain recoverable."""
...
def write_batch_summary_csvs(
    output_dir: Path,
    recording_rows: list[dict[str, Any]],
    epoch_rows: list[dict[str, Any]],
) -> tuple[Path, Path]:
    """Safely write recording-level and original-epoch batch QC summaries."""
    recording_path = output_dir / "batch_qc_summary.csv"
    epoch_path = output_dir / "batch_epoch_noise_metrics.csv"
    _write_csv_atomic(recording_path, recording_rows, BATCH_QC_COLUMNS)
    _write_csv_atomic(epoch_path, epoch_rows, BATCH_EPOCH_COLUMNS)
    return recording_path, epoch_path


def generate_batch_qc_plots(
    output_dir: Path,
    recording_rows: list[dict[str, Any]],
) -> tuple[list[Path], list[str]]:
    """Plot objective raw metrics and epoch retention across sessions by subject."""
    ...
    try:
        ...
        message = f"Batch QC plots were unavailable: {exc}"
        LOGGER.warning(message)
        return [], [message]

    output_dir.mkdir(parents=True, exist_ok=True)
    frame = pd.DataFrame(recording_rows)
    frame["session_numeric"] = pd.to_numeric(frame["session"], errors="coerce")
    frame["subject_plot"] = frame["subject"].replace("", "unknown")
    saved: list[Path] = []
    plot_warnings: list[str] = []
    plot_specs = [
        (
            "median_raw_50hz_ratio_db",
            "Median raw 50 Hz ratio (dB)",
            "Raw 50 Hz ratio by session",
            "batch_raw_50hz_ratio_by_session.png",
        ),
        (
            "median_raw_peak_to_peak_microvolts",
            "Median raw peak-to-peak amplitude (uV)",
            "Raw peak-to-peak amplitude by session",
            "batch_raw_peak_to_peak_by_session.png",
        ),
        (
            "retention_fraction",
            "Retained epoch fraction",
            "Epoch retention by session",
            "batch_epoch_retention_by_session.png",
        ),
    ]

    for metric, y_label, title, filename in plot_specs:
        fig = None
        try:
            plot_frame = frame.copy()
            plot_frame[metric] = pd.to_numeric(plot_frame[metric], errors="coerce")
            plot_frame = plot_frame.dropna(subset=["session_numeric", metric])
            if plot_frame.empty:
                raise ValueError(f"no valid session values for metric '{metric}'")
            grouped = (
                plot_frame.groupby(["subject_plot", "session_numeric"], as_index=False)[
                    metric
                ]
                .median()
                .sort_values(["subject_plot", "session_numeric"])
            )
            fig, ax = pyplot.subplots(figsize=(10, 5))
            for subject, subject_frame in grouped.groupby("subject_plot"):
                ax.plot(
                    subject_frame["session_numeric"],
                    subject_frame[metric],
                    marker="o",
                    linewidth=1.2,
                    label=str(subject),
                )
            ...
            ax.legend(title="Subject", fontsize="small", ncol=2)
            fig.tight_layout()
            output_path = output_dir / filename
            fig.savefig(output_path, dpi=150)
            saved.append(output_path)
        except Exception as exc:  # noqa: BLE001 - batch plots cannot stop processing.
            message = f"Batch plot '{filename}' failed: {exc}"
            plot_warnings.append(message)
            LOGGER.warning(message)
        finally:
            if fig is not None:
...
def print_summary(summary: ProcessingSummary) -> None:
    ...
    print(f"Retention fraction: {summary.retention_fraction:.6f}")
...
def main() -> None:
    ...
    batch_outputs_enabled = bool(
        args.batch and args.input_path.is_dir() and not args.dry_run
    )
    batch_recording_rows: list[dict[str, Any]] = []
    batch_epoch_rows: list[dict[str, Any]] = []
    if batch_outputs_enabled:
        write_batch_summary_csvs(
            args.output_dir,
            batch_recording_rows,
            batch_epoch_rows,
        )
    ...
    for input_file in input_files:
        ...
        if batch_outputs_enabled:
            recording_row, epoch_rows = build_batch_qc_rows(summary)
            batch_recording_rows.append(recording_row)
            batch_epoch_rows.extend(epoch_rows)
            write_batch_summary_csvs(
                args.output_dir,
                batch_recording_rows,
                batch_epoch_rows,
    ...
    if batch_outputs_enabled:
        generate_batch_qc_plots(args.output_dir, batch_recording_rows)
